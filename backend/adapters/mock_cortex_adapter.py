...
class MockCortexAdapter:
    ...
    def run_nl(self, nlq: str, dimension: str) -> List[Dict[str, Any]]:
        ...
        if dim == "time":
            df = self.tables["revenue_qoq"].sort_values("period").tail(2)
            return df.to_dict(orient="records")
        if dim == "region":
            df = self.tables["revenue_by_region"]
            latest = df["period"].max()
            return df[df["period"] == latest].sort_values("rev_delta_pct").to_dict(orient="records")
        if dim == "product":
            df = self.tables["revenue_by_product"]
            latest = df["period"].max()
            return df[df["period"] == latest].sort_values("rev_delta_pct").to_dict(orient="records")
        if dim == "orders":
            df = self.tables["orders_qoq"].sort_values("period").tail(2)
            return df.to_dict(orient="records")
        return []
